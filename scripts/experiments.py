...
data = pd.read_csv('Data/all_chords_ids.csv')
...
'''
# Tokenizaiton
data = pd.read_csv('Data/clean_full_lyrics.csv')
...
for i in range(len(lyrics)):
    ...
    lyrics[i] = [WordNetLemmatizer().lemmatize(token, get_wordnet_pos(token))
                 for token in lyrics[i]]
...
lyrical_similarities.to_csv('Data/l_similarities.csv')
...
for i in range(n):
    for j in range(i, n):
        ...
        dist_idx += 1
SM = pd.DataFrame(SM)
SM.to_csv('Data/m_similarities.csv', index=False)
# SM.to_csv('SM_levenshtein.csv', index=False)

chord_SM = pd.read_csv('Data/m_similarities.csv')
final_SM = pd.read_csv('Data/SM_levenshtein.csv')
...
def research_1(SL, SM):
    ...
    corr = np.dot(relevant_SL - mean_SL, relevant_SM - mean_SM) /\
        math.sqrt(np.sum(np.square(relevant_SL-mean_SL))
                  * np.sum(np.square(relevant_SM-mean_SM)))
    # Test significance
    t_score = corr * math.sqrt(len(relevant_SL) - 2) / \
        math.sqrt(1 - corr * corr)
...
def research_2(SL, SM, genres):
    ...
    pop_corr = np.dot(pop_SL - mean_pop_SL, pop_SM - mean_pop_SM) /\
        math.sqrt(np.sum(np.square(pop_SL-mean_pop_SL))
                  * np.sum(np.square(pop_SM-mean_pop_SM)))
    t_score = pop_corr * math.sqrt(len(pop_SL) - 2) / \
        math.sqrt(1 - pop_corr * pop_corr)
    p_value_pop = stats.t.sf(abs(t_score), df=len(pop_SL) - 2) * 2
    rock_corr = np.dot(rock_SL - mean_rock_SL, rock_SM - mean_rock_SM) /\
        math.sqrt(np.sum(np.square(rock_SL-mean_rock_SL))
                  * np.sum(np.square(rock_SM-mean_rock_SM)))
    t_score = rock_corr * \
        math.sqrt(len(rock_SL) - 2) / math.sqrt(1 - rock_corr * rock_corr)
    ...
    z_score = (z_pop - z_rock) / math.sqrt(1 /
                                           (len(pop_SL) - 3) + 1/(len(rock_SL) - 3))
...
def research_3(SL, SM, genres, artist_ids, alpha):
    ...
    for i in range(len(pop_same_corrs)):
        z_same = 0.5 * \
            math.log((1 + pop_same_corrs[i])/(1 - pop_same_corrs[i]))
        z_diff = 0.5 * \
            math.log((1 + pop_diff_corrs[i])/(1 - pop_diff_corrs[i]))
        z_score = (z_same - z_diff) / math.sqrt(1 /
                                                (len(pop_same_SL) - 3) + 1 / (len(pop_diff_SL) - 3))
        p_value = stats.norm.sf(abs(z_score)) * 2
        if p_value < alpha:
    ...
    for i in range(len(rock_same_corrs)):
        z_same = 0.5 * \
            math.log((1 + rock_same_corrs[i])/(1 - rock_same_corrs[i]))
        z_diff = 0.5 * \
            math.log((1 + rock_diff_corrs[i])/(1 - rock_diff_corrs[i]))
        z_score = (z_same - z_diff) / math.sqrt(1 /
                                                (len(rock_same_SL) - 3) + 1 / (len(rock_diff_SL) - 3))
        p_value = stats.norm.sf(abs(z_score)) * 2
        if p_value < alpha:
    ...
    z_score = (pop_phat - rock_phat) / math.sqrt(pop_phat *
                                                 (1 - rock_phat)*(1/len(pop_results) + 1/len(rock_results)))
    p_value = p_value = stats.norm.sf(abs(z_score)) * 2
    return (pop_phat, np.mean(pop_diffs)), (rock_phat, np.mean(rock_diffs)), p_value


# Remove songs whose artist has less than 3 songs
artist_ids = data['Artist_id'].to_numpy(dtype=int)
killrow_list = []
counter = 1
for i in range(1, len(artist_ids), 1):
...
lyrical_similarities.drop(
    lyrical_similarities.iloc[:, killrow_list], axis=1, inplace=True)
...
print('-----------------------------------------------------------------------')
print(research_3(lyrical_similarities, chord_SM,
      data['Genre'].tolist(), data['Artist_id'].tolist(), 0.05))
print(research_3(lyrical_similarities, final_SM,
      data['Genre'].tolist(), data['Artist_id'].tolist(), 0.05))
